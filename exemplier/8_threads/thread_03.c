 ...
 * creates NUM_THREADS threads. (NUM_THREADS-1) of those threads increment a
 * "count" variable, while the last one watches the value of "count".  When
 * "count" reaches a limit (COUNT_LIMIT), the waiting thread is signaled by
 ...
 * This example is adapted from example code in "Pthreads Programming",
